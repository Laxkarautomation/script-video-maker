...
function visualPrompt(scene, category, index) {
  const shots = ["wide shot", "closeup", "medium shot", "over the shoulder shot", "low angle shot", "dramatic object closeup"];
  const shot = shots[index % shots.length];

  return encodeURIComponent(
    `${category} story scene, cinematic realistic vertical 9:16, ${shot}, Indian environment, dramatic lighting, suspense mood, detailed background, emotional storytelling, no text, no watermark, no logo, no distorted face, no extra fingers. Scene: ${scene}`
  );
}
...
app.post("/api/generate", async (req, res) => {
  try {
    ...
    for (let i = 0; i < scenes.length; i++) {
      ...
      const url = `https://image.pollinations.ai/prompt/${visualPrompt(scenes[i], category, i)}?width=1080&height=1920&seed=${id}${i}&nologo=true`;
